...
PyObject *py_uwsgi_add_pin_event(PyObject * self, PyObject * args) {
	...
	uint8_t uwsgi_signal;
	int direction_is_rising, pin_number;

	if (!PyArg_ParseTuple(args, "Bii:add_pin_event", &uwsgi_signal, &direction_is_rising, &pin_number)) {
                return NULL;
        }

	if (uwsgi_signal_add_pin_event(uwsgi_signal, direction_is_rising, pin_number)) {
...
static PyMethodDef uwsgi_advanced_methods[] = {
	...
	{"add_pin_event", py_uwsgi_add_pin_event, METH_VARARGS, ""},  // --------------- rascal interrupts ----------------
